...
namespace Frontend.Pages.HistorialesPacientes
{
    public class CreateModel : PageModel
    {
        ...
        [BindProperty]
        public HistorialPaciente HistorialPaciente { get; set; } = new();
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            try
            {
                var response = await _httpClient.PostAsJsonAsync("api/HistorialPaciente", HistorialPaciente);

                if (response.IsSuccessStatusCode)
                {
                    var result = await response.Content.ReadFromJsonAsync<ResponseCreate>();
                    Mensaje = result?.mensaje ?? "Historial del paciente creado exitosamente";
                    return RedirectToPage("Index");
                }
                else
                {
                    var error = await response.Content.ReadFromJsonAsync<ResponseError>();
                    ModelState.AddModelError(string.Empty, error?.mensaje ?? "Error al crear el historial del paciente");
                    await LoadPacientes();
                    return Page();
                }
            }
        ...
        {
            public string mensaje { get; set; }
            public HistorialPaciente historialPaciente { get; set; }
        }
